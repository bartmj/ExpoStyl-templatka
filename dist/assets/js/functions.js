...
"use strict";
    ...
    // BEGIN: 03 Helper functions for my modifications of porfolio isotope 
    // Elements being detached when isotope filter is used
    const detachedElementsArray = [];
    // A function to appear all the detached elements
    function reappear() {
        for (let i = 0; i < detachedElementsArray.length; i++) {
            $('.root-element').append(detachedElementsArray[i])
        }
    } 
    ...
    var portfolioIsotope = function() {
        ...
        if ( $().isotope ) {
            ...
            $('.nav-tabs .nav-filter').on('click',function() {
                ...
                // Appending detached earlier elements
                reappear()
                // Makeing clicked class active
                $('.nav-tabs .nav-filter').removeClass('active');
                $(this).addClass('active');
                // The isotope filtering happens here
                $container.isotope({ filter: selector });
                // Detach all the elements 
                for (let i = 0; i < dataFilters.length; i++) {
                    if(selector.includes('*')) {
                        console.log("SUCCESS")
                    } else if (!selector.includes(dataFilters[i])) {
                        const detachedElement = $('.' + dataFilters[i]).detach();
                        detachedElementsArray.push(detachedElement);
                    }
                }
                return false;
            });
    ...
    var wow = function() {
        if($('.wow').length){
            wow = new WOW( {
                mobile: false  // Turn on/off WOW.js on mobile devices.
            }).init();
        }
    };
    // END: Wow Js
